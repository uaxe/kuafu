...
func (s *CMCCProvider) telnetAdminAndPwd(macAddr string, ret *modem.SuperAdmin) error {
	...
	uname := "admin"
	pwd := fmt.Sprintf("Fh@%s", macAddr[len(macAddr)-6:])
	...
	n, err = conn.Write([]byte("load_cli factory\n"))
	if err != nil {
		return err
	}
	...
	n, err = conn.Write([]byte("show admin_name\n"))
	if err != nil {
		return err
	}
	...
	n, err = conn.Write([]byte("show admin_pwd\n"))
	if err != nil {
		return err
	}
